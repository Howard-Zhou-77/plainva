...
import type { SearchPageCursor, SearchResult, VaultQueryService } from "@plainva/core";

const EMPTY: SearchResult[] = [];
type SearchService = Pick<VaultQueryService, "searchOccurrencesPage">;
interface State { query: string; service: SearchService | null | undefined; revision: number; hits: SearchResult[]; next: SearchPageCursor | null; loading: boolean; failed: boolean }

/** One cancellation and paging contract for all three search surfaces. */
export function useSearchPages(service: SearchService | null | undefined, query: string, revision = 0, limit = 40) {
  const request = useRef<AbortController | null>(null);
  const [state, setState] = useState<State>({ query: "", service, revision, hits: EMPTY, next: null, loading: false, failed: false });
  const load = useCallback((cursor: SearchPageCursor | null) => {
    request.current?.abort();
    const controller = new AbortController();
    request.current = controller;
    const initial: State = { query, service, revision, hits: EMPTY, next: null, loading: true, failed: false };
    ...
    void service.searchOccurrencesPage(query, { cursor, limit, signal: controller.signal }).then((page) => {
  ...
  }, [service, query, revision, limit]);
  useEffect(() => { load(null); return () => request.current?.abort(); }, [load]);
  const current = state.query === query && state.service === service && state.revision === revision;
  return {
